...
function load(): AuthState {
  try {
    ...
    if (!raw) return { role: null, patientId: null, doctorId: null }
    const parsed = JSON.parse(raw) as Partial<AuthState>
    return {
      role: parsed.role === 'doctor' || parsed.role === 'patient' ? parsed.role : null,
    ...
    return { role: null, patientId: null, doctorId: null }
  }
}
